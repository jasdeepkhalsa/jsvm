...
class CPU {
  constructor(memory) {
    ...
    this.registers = createMemory(this.registerNames.length * 2) // TODO: Why 2?
  ...
  viewMemoryAtAddress(address) {
    ...
    console.log(
      `${address.toString(16).padStart(4, '0')}: ${nextEightBytes.join(' ')}`
    )
  }
  ...
  executeInstruction(instruction) {
    switch (instruction) {
      case Instruction.MOVE_LITERAL_REGISTER: {
        ...
        const register =
          (this.fetch8BitInstruction() % this.registerNames.length) * 2
      ...
      case Instruction.ADD_REGISTER_REGISTER: {
        ...
        const register1Value = this.registers.getUint16(register1 * 2)
        const register2Value = this.registers.getUint16(register2 * 2)
